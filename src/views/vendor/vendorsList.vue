...
import api from "@/api/axiosInstance";   // ✅ 使用自訂 axiosInstance
...
const fetchVendors = async () => {
  try {
    ...
    try {
      // ✅ 收藏 API 還是走 /api
      const favoritesRes = await api.get("/api/favorites/my");
    ...
    vendors.value = vendorsRes.data.map((v) => ({
      ...v,
      isFavorited: favoriteIds.has(v.vendorId),
    }));
  } catch (err) {
    console.error(err);
  }
...
const filteredVendors = computed(() => {
  return vendors.value.filter((v) => {
    const matchesSearch =
      v.vendorName.toLowerCase().includes(searchText.value.toLowerCase()) ||
